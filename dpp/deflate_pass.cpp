//
// Created by Марк Лемешевский on 16.07.2020.
//

#include "deflate_pass.hpp"

auto dpp::internal::get_length_index(int32_t length) -> uint32_t
{
    if (length < 11)
    {
        return 257 + length - 3;
...
auto dpp::internal::get_offset_index(int32_t offset) -> uint32_t
{
    if (offset <= 2)
    {
        return offset - 1;
